...
def write_bot_config(cfg, markets) -> None:
    ...
    tick_size = (m.get("info",{}) or {}).get("tickSize") or (m.get("precision",{}) or {}).get("price")
    step_size = (m.get("info",{}) or {}).get("stepSize") or (m.get("precision",{}) or {}).get("amount")
...
def write_candles_with_signals(cfg, candles, fast_series, slow_series) -> None:
    opens=[c[1] for c in candles]; highs=[c[2] for c in candles]; lows=[c[3] for c in candles]
    closes=[c[4] for c in candles]; vols=[c[5] for c in candles]
    sigs=[]; off_f=len(closes)-len(fast_series); off_s=len(closes)-len(slow_series)
    for i in range(len(candles)):
        ...
        sigs.append({
            ...
            "volume": float(vols[i]),
            "fast_sma": (None if f is None or i<off_f else float(fast_series[i-off_f])),
            "slow_sma": (None if s is None or i<off_s else float(slow_series[i-off_s])),
            "signal": sig
        })
...
def append_snapshot(cfg, state) -> None:
    ...
    if len(snaps) > 5000: snaps = snaps[-5000:]
