...
class SimplePhase4Test:
            ...
            for symbol in statistics.get("symbols", []):
                for interval in statistics.get("intervals", []):
                    ...
                    if self.use_real_data:
                        ...
                        sample_data = await self.data_connector.get_symbol_data(
                            symbol, interval, limit=100
                        )
                    else:
                        # Use sync method for test data
                        sample_data = self.data_connector.get_symbol_data(
                            symbol, interval, limit=100
                        )
                    ...
                    if sample_data is not None and not sample_data.empty:
                        test_results[f"{symbol}_{interval}"] = {
                            ...
                            "data_quality": "good",
                            "data_source": (
                                "real_data" if self.use_real_data else "test_data"
                            ),
                        }
                        logger.info(
                            f"✅ {symbol} {interval}: {len(sample_data)} data points ({'real' if self.use_real_data else 'test'} data)"
                        )
                    else:
                        test_results[f"{symbol}_{interval}"] = {
                            "status": "error",
    ...
    async def _test_basic_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.use_real_data:
                btc_data = await self.data_connector.get_symbol_data(
                    "BTCUSDT", "1h", limit=500
                )
                eth_data = await self.data_connector.get_symbol_data(
                    "ETHUSDT", "1h", limit=500
                )
            else:
                btc_data = self.data_connector.get_symbol_data(
                    "BTCUSDT", "1h", limit=500
                )
                eth_data = self.data_connector.get_symbol_data(
                    "ETHUSDT", "1h", limit=500
                )
        ...
        try:
            ...
            if len(close_prices) >= 20:
                ...
                trend = (
                    "bullish"
                    if short_ma > long_ma
                    else "bearish" if short_ma < long_ma else "neutral"
                )
            else:
            ...
            return {
                ...
                "trend_analysis": {"trend": trend, "data_points": len(close_prices)},
            }
    ...
    async def _test_strategy_simulation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if btc_data is None or btc_data.empty:
                return {
                    "status": "error",
                    "error": "Failed to get BTC data for strategy testing",
                }
        ...
        try:
            ...
            return {
                ...
                "deviation_percent": round(
                    abs(current_price - mean_price) / mean_price * 100, 2
                ),
            }
    ...
    def _simulate_momentum(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            price_change_1h = (
                (close_prices[-1] - close_prices[-2]) / close_prices[-2] * 100
            )
            price_change_5h = (
                (close_prices[-1] - close_prices[-6]) / close_prices[-6] * 100
    ...
    async def _test_bot_management(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_bots = sum(
                1 for bot in bot_status.values() if bot["status"] == "active"
            )
            ...
            return {
                ...
                "portfolio_metrics": {
                    ...
                    "roi_percent": (
                        round(total_pnl / total_capital * 100, 2)
                        if total_capital > 0
                        else 0
                    ),
                },
