...
def _desired_usd(state: dict, price: float, default_usd: float) -> float:
    ...
    try:
        ...
        pct = 0.0
    if pct > 0:
        equity = float(state.get("cash_usd", 0.0)) + float(state.get("coin_units", 0.0)) * float(price)
...
def sell(state: dict, price: float, fee_rate: float) -> dict:
    ...
    return {"ok": True, "units": units, "fee": fee, "proceeds": proceeds, "pnl_gross": pnl_gross, "pnl_net": pnl_net, "exit_price": price}
