...
class DataPreprocessor:
    ...
    def generate_synthetic_data(self, days: int = 365, base_price: float = 100.0, 
                               volatility: float = 0.02, trend: float = 0.0001) -> List[OHLCVData]:
        ...
        for i in range(days):
            ...
            close_price = current_price * (1 + np.random.normal(0, daily_volatility * 0.3))
            high_price = max(open_price, close_price) * (1 + abs(np.random.normal(0, daily_volatility * 0.2)))
            low_price = min(open_price, close_price) * (1 - abs(np.random.normal(0, daily_volatility * 0.2)))
        ...
        for i, record in enumerate(data):
            try:
                ...
                if record.high < max(record.open, record.close):
                    errors.append(f"Record {i}: High price ({record.high}) < max(open, close)")
                    continue
                
                if record.low > min(record.open, record.close):
                    errors.append(f"Record {i}: Low price ({record.low}) > min(open, close)")
                    continue
    ...
    def clean_data(self, data: List[OHLCVData], 
                   remove_outliers: bool = True,
                   outlier_threshold: float = 3.0,
                   fill_gaps: bool = True) -> List[OHLCVData]:
        ...
        if remove_outliers:
            ...
            price_threshold = df['price_change'].mean() + outlier_threshold * df['price_change'].std()
            
            # Calculate volume outliers
            volume_threshold = df['volume'].mean() + outlier_threshold * df['volume'].std()
            
            # Remove outliers
            outliers_mask = (df['price_change'] > price_threshold) | (df['volume'] > volume_threshold)
        ...
        if fill_gaps:
            ...
            expected_interval = time_diffs.mode().iloc[0] if len(time_diffs) > 1 else 86400000  # Default to 1 day
            ...
            if gaps.any():
                gap_count = gaps.sum()
                logger.info(f"Found {gap_count} time gaps, filling with interpolation...")
        ...
        logger.info(f"Data cleaning complete: {len(cleaned_data)}/{original_count} records retained")
        return cleaned_data
    ...
    def calculate_technical_indicators(self, data: List[OHLCVData]) -> Dict[str, List[float]]:
        ...
        indicators['volume_ratio'] = [v / indicators['volume_sma'][i] if i < len(indicators['volume_sma']) else 1.0 
    ...
    def _calculate_bollinger_bands(self, data: List[float], period: int, 
                                  std_dev: float) -> Tuple[List[float], List[float]]:
    ...
    def export_data(self, data: List[OHLCVData], indicators: Dict[str, List[float]], 
                    filename: str) -> bool:
