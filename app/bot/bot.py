...
def ensure_expected_files_exist(state: Dict[str, Any]):
    for p, d in [
        (STATE_PATH, state),
        (TRADES_PATH, []),
        (TRADES_DETAILED_PATH, []),
        (CANDLES_WITH_SIGNALS_PATH, []),
        (SNAP_PATH, []),
        (BOTCFG_PATH, {}),
    ]:
...
def load_profile() -> Dict[str, Any]:
    ...
    cfg = {
        ...
        "ORDER_PCT_EQUITY": (
            (
                None
                if os.environ.get("ORDER_PCT_EQUITY") in (None, "", "null")
                else float(os.environ.get("ORDER_PCT_EQUITY"))
            )
            if ("ORDER_PCT_EQUITY" in os.environ and ALLOW_ENV)
            else profile.get("ORDER_PCT_EQUITY")
        ),
        ...
        "REBALANCE_TARGET_STASH_PCT": float(
            profile.get("REBALANCE_TARGET_STASH_PCT", 0.70)
        ),
...
def main():
    print(
        f"[bot] start {CFG['EXCHANGE']}:{SYMBOL} tf={TIMEFRAME} F/S={CFG['FAST']}/{CFG['SLOW']} fee={CFG['FEE_RATE']} profile={CFG.get('PROFILE')}"
    )
    ...
    while True:
        try:
            ...
            o = [c[1] for c in closed]
            h = [c[2] for c in closed]
            low = [c[3] for c in closed]
            c = [c[4] for c in closed]
            v = [c[5] for c in closed]
            ...
            spread = (
                abs((fast[-1] if fast[-1] is not None else last_price) - anchor)
                / last_price
            )
            if spread <= CFG["THRESHOLD_PCT"]:
                state["last_action"] = "skip"
                state["skip_reason"] = f"hysteresis<{CFG['THRESHOLD_PCT']}"
                state["last_price"] = last_price
                state["unrealized_pnl_usd"] = (
                    last_price - (state.get("entry_price") or last_price)
                ) * float(state["trade_coin_units"])
                state["coin_units"] = float(
                    state["trade_coin_units"] + state["stash_coin_units"]
                )
                state["equity_usd"] = equity_usd(state, last_price)
                state["updated_at"] = iso(last_ts)
                atomic_write_json(STATE_PATH, state)
                append_json_array(
                    SNAP_PATH, {"ts": iso(last_ts), "equity_usd": state["equity_usd"]}
                )
                ensure_expected_files_exist(state)
                append_json_array(
                    CANDLES_WITH_SIGNALS_PATH,
                ...
                botcfg = {
                    ...
                    "maker_fee_bp": CFG["FEE_RATE"] * 10000,
                    "taker_fee_bp": CFG["FEE_RATE"] * 10000,
                    "assumed_slippage_bp": CFG["SLIPPAGE_BP"],
                    "min_notional_usd": 1.0,
                    "tick_size": 0.01,
                    "step_size": 1e-5,
                    "stack_floor_usd": CFG["STACK_FLOOR_USD"],
                    "retain_pct_up": CFG["RETAIN_PCT_UP"],
                    "retain_pct_chop": CFG["RETAIN_PCT_CHOP"],
                    "retain_pct_down": CFG["RETAIN_PCT_DOWN"],
            ...
            in_uptrend = (
                slope >= CFG["SLOPE_MIN_PCT_PER_BAR"]
                if CFG["SLOPE_MIN_PCT_PER_BAR"] > 0
                else True
            )
            ...
            if (
                last_trade_close_ts
                and (last_ts - int(last_trade_close_ts)) < CFG["MIN_HOLD_BARS"] * tfms
            ):
                state["last_action"] = "skip"
            ...
            if signal == "sell":
                retain_pct = (
                    CFG["RETAIN_PCT_UP"] if in_uptrend else CFG["RETAIN_PCT_CHOP"]
                )

            # retain throttle by cash ratio
            cash_ratio = float(state["cash_usd"]) / max(
                1e-9, equity_usd(state, last_price)
            )
            ...
            if signal == "buy":
                        ...
                        if cost < CFG["MIN_TRADE_USD"]:
                            ...
                            append_json_array(
                                TRADES_PATH,
                                {
                                    ...
                                    "units": qty,
                                    "fee_usd": round(fee, 6),
                                    "cash_usd": round(state["cash_usd"], 6),
                                    "coin_units": round(
                                        state["trade_coin_units"]
                                        + state["stash_coin_units"],
                                        12,
                                    ),
                                },
                            )
                ...
                append_json_array(
                    TRADES_PATH,
                    {
                        ...
                        "cash_usd": round(state["cash_usd"], 6),
                        "coin_units": round(
                            state["trade_coin_units"] + state["stash_coin_units"], 12
                        ),
                        "pnl": round(realized_pnl, 6),
                        "note": f"regime={'up' if in_uptrend else 'chop'}",
                    },
                )
                append_json_array(
                    TRADES_DETAILED_PATH,
