...
class DeploymentTester:
    ...
    def test_container_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                "tested": sum(
                    1 for c in container_results.values() if c["status"] != "not_tested"
                ),
                "success": sum(
                    1 for c in container_results.values() if c["status"] == "success"
                ),
                "warning": sum(
                    1 for c in container_results.values() if c["status"] == "warning"
                ),
                "error": sum(
                    1 for c in container_results.values() if c["status"] == "error"
                ),
            },
        }
    ...
    def print_summary(self, summary: Dict[str, Any]):
        ...
        print(
            f"📊 Success Rate: {summary['success_rate']}% ({summary['successful_endpoints']}/{summary['total_endpoints']})"
        )
        print()
        ...
        for result in summary["results"]:
            ...
            print(
                f"{status_icon} {result['name']:<20} {result['status']:<15} {response_time}"
            )
        ...
        if (
            "phase4_components" in summary
            and summary["phase4_components"]["status"] == "completed"
        ):
            print()
        ...
        if (
            "container_status" in summary
            and summary["container_status"]["status"] == "completed"
        ):
            print()
    ...
    def test_phase4_components(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if history_result["status"] == "success":
                if (
                    ...
                    print(
                        "    ⚠️  No historical data available (may be normal for new deployment)"
                    )
            else:
                phase4_results["data_connector"]["status"] = "error"
                phase4_results["data_connector"][
                    "error"
                ] = f"History endpoint failed: {history_result.get('error')}"
            ...
            if real_data_result["status"] == "success":
                ...
                phase4_results["real_data_access"]["error"] = real_data_result.get(
                    "error", "Limited real data access"
                )
                print(
                    f"    ⚠️  Limited real data access: {real_data_result.get('error')}"
                )
            else:
                phase4_results["real_data_access"]["status"] = "error"
                phase4_results["real_data_access"]["error"] = real_data_result.get(
                    "error", "Real data access failed"
                )
                print(
                    f"    ❌ Real data access failed: {real_data_result.get('error')}"
                )
            ...
            if health_result["status"] == "success" and health_result["data"]:
                ...
                if data.get("status") == "enhanced":
                    ...
                    print(
                        "    ⚠️  Basic system monitoring (enhanced features may not be deployed)"
                    )
            ...
            perf_result = self.test_endpoint(
                "system_performance", "/api/system/performance"
            )
        ...
        return {
            ...
            "summary": {
                "tested": sum(
                    1 for c in phase4_results.values() if c["status"] != "not_tested"
                ),
                "success": sum(
                    1 for c in phase4_results.values() if c["status"] == "success"
                ),
                "warning": sum(
                    1 for c in phase4_results.values() if c["status"] == "warning"
                ),
                "error": sum(
                    1 for c in phase4_results.values() if c["status"] == "error"
                ),
            },
        }
