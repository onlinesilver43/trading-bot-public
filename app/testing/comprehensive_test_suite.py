...
class ComprehensiveTestSuite:
    ...
    def _test_bot_system(self):
        ...
        try:
            ...
            try:
                ...
                self._record_test_result(component, "Bot Main Import", "PASS", duration,
                                       "Successfully imported bot main function")
                
                # Test bot utility functions
                start_time = time.time()
                from bot.bot import load_profile, get_exchange, sma, slope_pct_per_bar
                duration = time.time() - start_time
                
                self._record_test_result(component, "Bot Utilities Import", "PASS", duration,
                ...
                try:
                    ...
                    if profile:
                        self._record_test_result(component, "Profile Loading", "PASS", 0,
                                               "Successfully loaded bot profile")
                    else:
                        self._record_test_result(component, "Profile Loading", "SKIP", 0,
                ...
                self._record_test_result(component, "Bot Import", "ERROR", 0,
                                       f"Import error: {e}")
    ...
    def _test_ui_system(self):
        ...
        try:
            ...
            self._record_test_result(component, "UI Import", "PASS", duration,
                                   "Successfully imported FastAPI app")
            
            # Test enhanced UI import
            start_time = time.time()
            from ui.ui_enhanced import get_enhanced_system_health, get_enhanced_system_resources
            duration = time.time() - start_time
            
            self._record_test_result(component, "Enhanced UI Import", "PASS", duration,
            ...
            self._record_test_result(component, "UI Routes Import", "PASS", duration,
                                   "Successfully imported UI app")
            
            # Test UI helpers import
            start_time = time.time()
            from ui import load_json, zip_dirs
            duration = time.time() - start_time
            
            self._record_test_result(component, "UI Helpers Import", "PASS", duration,
                                   "Successfully imported UI helper functions")
            
            # Test FastAPI app functionality
            start_time = time.time()
            if hasattr(app, 'routes') and len(app.routes) > 0:
                self._record_test_result(component, "FastAPI Routes", "PASS", 0,
                                       f"FastAPI app has {len(app.routes)} routes")
            else:
                self._record_test_result(component, "FastAPI Routes", "FAIL", 0,
                                       "FastAPI app has no routes")
    ...
    def _test_core_trading_logic(self):
        ...
        try:
            ...
            self._record_test_result(component, "Core Utils Import", "PASS", duration,
            ...
            if sma and len(sma) > 0:
                self._record_test_result(component, "SMA Calculation", "PASS", duration,
                                       "Successfully calculated SMA")
            else:
                self._record_test_result(component, "SMA Calculation", "FAIL", duration,
            ...
            if now_str and ms_time == 300000:  # 5 minutes = 300,000 ms
                self._record_test_result(component, "Time Utilities", "PASS", duration,
                                       "Successfully tested time utility functions")
            else:
                self._record_test_result(component, "Time Utilities", "FAIL", duration,
    ...
    def _test_exchange_integration(self):
        ...
        try:
            ...
            self._record_test_result(component, "Exchange Import", "PASS", duration,
            ...
            try:
                # Test ccxt import
                import ccxt
                self._record_test_result(component, "CCXT Import", "PASS", 0,
                                       f"Successfully imported ccxt version {ccxt.__version__}")
                
                # Test exchange client creation
                client = Client("binanceus")
                self._record_test_result(component, "Client Creation", "PASS", 0,
                ...
                try:
                    ...
                    if markets:
                        self._record_test_result(component, "Markets Loading", "PASS", 0,
                                               f"Successfully loaded {len(markets)} markets")
                    else:
                        self._record_test_result(component, "Markets Loading", "SKIP", 0,
                ...
                self._record_test_result(component, "CCXT Functionality", "FAIL", 0,
    ...
    def _test_portfolio_management(self):
        ...
        try:
            ...
            self._record_test_result(component, "Portfolio Import", "PASS", duration,
            ...
            try:
                ...
                if buy_result["ok"]:
                    self._record_test_result(component, "Buy Function", "PASS", 0,
                                           f"Buy function successful: {buy_result['units']:.8f} units")
                else:
                    self._record_test_result(component, "Buy Function", "FAIL", 0,
                ...
                if sell_result["ok"]:
                    self._record_test_result(component, "Sell Function", "PASS", 0,
                                           f"Sell function successful: PnL {sell_result['pnl_net']:.2f}")
                else:
                    self._record_test_result(component, "Sell Function", "FAIL", 0,
                                           f"Sell function failed: {sell_result['reason']}")
                
                # Test can_spend function
                spend_result = can_spend(1000.0, 0.001, 50000.0, 100.0)
                if spend_result > 0:
                    self._record_test_result(component, "Can Spend Function", "PASS", 0,
                                           f"Can spend function working: {spend_result:.8f} units")
                else:
                    self._record_test_result(component, "Can Spend Function", "FAIL", 0,
                                           "Can spend function returned 0 or negative")
                
            except Exception as e:
                self._record_test_result(component, "Portfolio Functionality", "FAIL", 0,
    ...
    def _test_state_management(self):
        ...
        try:
            ...
            self._record_test_result(component, "State Import", "PASS", duration,
            ...
            try:
                ...
                if Path(test_file).exists():
                    self._record_test_result(component, "Save JSON", "PASS", 0,
                                           "Successfully saved JSON file")
                else:
                    self._record_test_result(component, "Save JSON", "FAIL", 0,
                                           "Failed to save JSON file")
                
                # Test load_json
                loaded_data = load_json(test_file, {})
                if loaded_data == test_data:
                    self._record_test_result(component, "Load JSON", "PASS", 0,
                                           "Successfully loaded JSON data")
                else:
                    self._record_test_result(component, "Load JSON", "FAIL", 0,
                ...
                if 'symbol' in updated_state and 'rules' in updated_state:
                    self._record_test_result(component, "Ensure Defaults", "PASS", 0,
                                           "Successfully applied default state values")
                else:
                    self._record_test_result(component, "Ensure Defaults", "FAIL", 0,
                ...
                self._record_test_result(component, "State Functionality", "FAIL", 0,
    ...
    def _test_phase3_components(self):
        ...
        try:
            ...
            self._record_test_result(component, "Import Test", "ERROR", 0,
                                   f"Error importing: {e}")
    ...
    def _test_system_integration(self):
        ...
        try:
            ...
            data = preprocessor.generate_synthetic_data(days=120)  # Need at least 100 for regime detection
            ...
            if regime and fast_sma and slow_sma:
                self._record_test_result(component, "Data Flow Integration", "PASS", duration,
                                       "Successfully tested data flow integration")
            else:
                self._record_test_result(component, "Data Flow Integration", "FAIL", duration,
    ...
    def _test_api_endpoints(self):
        ...
        try:
            ...
            self._record_test_result(component, "App Import", "PASS", duration,
                                   "Successfully imported FastAPI app")
            
            # Check if app has routes
            if hasattr(app, 'routes') and len(app.routes) > 0:
                self._record_test_result(component, "Routes Available", "PASS", 0,
                                       f"App has {len(app.routes)} routes")
            else:
                self._record_test_result(component, "Routes Available", "FAIL", 0,
                                       "App has no routes")
            ...
            try:
                ...
                if hasattr(app, 'openapi'):
                    ...
                    if openapi_schema and 'paths' in openapi_schema:
                        self._record_test_result(component, "OpenAPI Schema", "PASS", 0,
                                               f"OpenAPI schema has {len(openapi_schema['paths'])} paths")
                    else:
                        self._record_test_result(component, "OpenAPI Schema", "FAIL", 0,
                                               "OpenAPI schema is missing or invalid")
                else:
                    self._record_test_result(component, "OpenAPI Schema", "SKIP", 0,
    ...
    def _test_file_operations(self):
        ...
        try:
            ...
            if Path(test_file).exists():
                self._record_test_result(component, "File Creation", "PASS", duration,
                ...
                if not Path(test_file).exists():
                    self._record_test_result(component, "File Deletion", "PASS", duration,
                                           "Successfully deleted test file")
                else:
                    self._record_test_result(component, "File Deletion", "FAIL", duration,
                                           "Failed to delete test file")
            else:
                self._record_test_result(component, "File Creation", "FAIL", duration,
    ...
    def _generate_test_report(self) -> Dict[str, Any]:
        ...
        for component, status in component_summary.items():
            print(f"   {status['overall_status']} {component}: {status['overall_status']}")
