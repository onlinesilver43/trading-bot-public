...
class Phase3TestSuite:
    ...
    def _test_market_regime_detection(self):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, "market_analysis/test_regime_detection.py"],
                capture_output=True,
                text=True,
                timeout=30,
            )
    ...
    def _test_strategy_module(self):
        ...
        try:
            ...
            fast_sma, slow_sma = indicators(
                test_prices, fast=10, slow=20, closed_only=True
            )
            duration = time.time() - start_time
            ...
            result = subprocess.run(
                [sys.executable, "test_strategy.py"],
                capture_output=True,
                text=True,
                timeout=30,
            )
    ...
    def _test_performance_database(self):
        ...
        try:
            ...
            metrics = db.calculate_performance(
                "TestStrategy", "TEST/USD", "1h", "2024-01-01", "2024-12-31"
            )
            duration = time.time() - start_time
    ...
    def _test_data_preprocessing(self):
        ...
        try:
            ...
            if len(cleaned_data) > 0:
                ...
                self._record_test_result(
                    component, "Data Cleaning", "FAIL", duration, "Data cleaning failed"
                )
    ...
    def _record_test_result(
        ...
        logger.info(
            f"{status_emoji.get(status, '❓')} {component} - {test_name}: {status}"
        )
        if message:
