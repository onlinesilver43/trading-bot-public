...
def create_test_market_data():
    ...
    for i in range(200):
        timestamp = int(
            (datetime.now() - timedelta(minutes=200 - i)).timestamp() * 1000
        )
...
def test_regime_detection():
    ...
    try:
        ...
        print("\nPerformance Summary:")
        print(
            f"  Total Detections: {summary.get('performance', {}).get('total_detections', 0)}"
        )
        print(
            f"  Regime Changes: {summary.get('performance', {}).get('regime_changes', 0)}"
        )
        print(
            f"  Average Confidence: {summary.get('performance', {}).get('avg_confidence', 0):.3f}"
        )
        ...
        successful_detections = sum(
            1 for result in results.values() if result["regime"] != "unknown"
        )
