...
Tests the new separate deployment workflows to ensure they work correctly
"""
...
class DeploymentWorkflowTester:
    ...
    def test_workflow_triggers(self) -> Dict[str, Any]:
        ...
        if core_workflow.exists():
            ...
            if 'paths:' in content and 'app/**' in content:
                trigger_results["deploy-core"] = "✅ Has path filters for app/** and compose/**"
            else:
                trigger_results["deploy-core"] = "❌ Missing path filters for app/** and compose/**"
        ...
        if data_workflow.exists():
            ...
            if 'paths:' in content and 'history_fetcher/**' in content:
                trigger_results["deploy-data"] = "✅ Has path filters for data components"
            else:
                trigger_results["deploy-data"] = "❌ Missing path filters for data components"
        ...
        if config_workflow.exists():
            ...
            if 'paths:' in content and 'config/**' in content:
                trigger_results["deploy-config"] = "✅ Has path filters for config components"
            else:
                trigger_results["deploy-config"] = "❌ Missing path filters for config components"
        
        return {
            "status": "success" if all("✅" in r for r in trigger_results.values()) else "warning",
    ...
    def test_data_preservation_logic(self) -> Dict[str, Any]:
        ...
        if core_workflow.exists():
            ...
            if '--delete' in content and 'history' not in content and 'data' not in content:
                preservation_results["deploy-core"] = "✅ Only updates app/compose, preserves data"
            else:
        ...
        if data_workflow.exists():
            ...
            if 'preserve collected data' in content:
                preservation_results["deploy-data"] = "✅ Explicitly preserves collected data"
            else:
                preservation_results["deploy-data"] = "⚠️  No explicit data preservation message"
        
        return {
            "status": "success" if all("✅" in r for r in preservation_results.values()) else "warning",
    ...
    def test_health_checks(self) -> Dict[str, Any]:
        ...
        return {
            "status": "success" if all("✅" in r for r in health_check_results.values()) else "warning",
    ...
    def save_report(self):
        ...
        timestamp = subprocess.run(["date", "-u", "+%Y%m%d_%H%M%S"], 
