...
def test_component(component_name: str) -> bool:
    ...
    try:
        if component_name == "regime":
            ...
            print(
                "Available components: regime, strategy, database, preprocessing, backtesting, all"
            )
            return False
...
def _test_strategy() -> bool:
    ...
    try:
        ...
        if len(fast_sma) > 0 and len(slow_sma) > 0:
            print(
                f"✅ Strategy indicators working: {len(fast_sma)} fast, {len(slow_sma)} slow SMA"
            )
...
def _test_backtesting() -> bool:
    ...
    try:
        ...
        # Create engine
        # engine = BacktestingEngine(config)  # Not currently used
