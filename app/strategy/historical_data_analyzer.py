...
class HistoricalDataAnalyzer:
    ...
    def __init__(self, 
        ...
        logger.info(f"Historical Data Analyzer initialized for {len(self.symbols)} symbols")
    ...
    async def _pull_historical_data(self):
        ...
        for symbol in self.symbols:
            ...
            try:
                ...
                if symbol_data:
                    ...
                    logger.info(f"✅ {symbol}: Data collected for {len(symbol_data)} timeframes")
                else:
        ...
        logger.info(f"Data collection complete: {len(self.historical_data)} symbols with data")
    
    async def _pull_symbol_data(self, symbol: str, timeframe: str) -> Optional[pd.DataFrame]:
    ...
    async def _load_from_existing_sources(self, symbol: str, timeframe: str) -> Optional[pd.DataFrame]:
    ...
    async def _generate_synthetic_data(self, symbol: str, timeframe: str) -> pd.DataFrame:
        ...
        try:
            ...
            returns = np.random.normal(0.0001, 0.02, data_points)  # Small returns per timeframe
            prices = [base_price]
            ...
            logger.info(f"Generated {len(df)} synthetic data points for {symbol} {timeframe}")
            return df
    ...
    async def _analyze_market_regimes(self):
        ...
        for symbol, timeframes_data in self.historical_data.items():
            ...
            for timeframe, data in timeframes_data.items():
                try:
                    regime_data = await self._analyze_single_regime(symbol, timeframe, data)
                    if regime_data:
                        symbol_regimes[timeframe] = regime_data
                        logger.info(f"  {timeframe}: {regime_data.regime} (confidence: {regime_data.confidence:.2f})")
                    
                except Exception as e:
                    logger.error(f"Error analyzing {timeframe} regime for {symbol}: {str(e)}")
            
            if symbol_regimes:
                self.regime_analysis[symbol] = symbol_regimes
                logger.info(f"✅ {symbol}: Regime analysis complete for {len(symbol_regimes)} timeframes")
        
        logger.info(f"Regime analysis complete: {len(self.regime_analysis)} symbols analyzed")
    
    async def _analyze_single_regime(self, symbol: str, timeframe: str, data: pd.DataFrame) -> Optional[MarketRegimeData]:
        ...
        try:
            ...
            mean_reversion_score = 1.0 / (1.0 + price_range)  # Higher score = more mean reversion
    ...
    def _classify_regime(self, volatility: float, trend_strength: float, 
                        mean_reversion: float, momentum: float) -> Tuple[str, float]:
    ...
    def _calculate_grid_trading_score(self, data: pd.DataFrame) -> float:
        ...
        try:
            ...
            # Combine factors
            grid_score = (volatility_consistency * 0.4 + 
                         (1.0 - price_range) * 0.3 + 
            ...
            for timeframe, regime_data in timeframes_data.items():
                try:
                    ...
                    if symbol_opportunities:
                        logger.info(f"  {timeframe}: Found {len(symbol_opportunities)} opportunities")
                    
                except Exception as e:
                    logger.error(f"Error finding opportunities for {symbol} {timeframe}: {str(e)}")
        
        self.strategy_opportunities = opportunities
        logger.info(f"Strategy opportunities identified: {len(opportunities)} total")
    
    def _find_regime_opportunities(self, symbol: str, timeframe: str, 
                                 regime_data: MarketRegimeData) -> List[StrategyOpportunity]:
            ...
            if "sideways" in regime or "mean_reversion" in regime:
                if regime_data.mean_reversion_score > 0.6:
                    opportunities.append(StrategyOpportunity(
                        ...
                        opportunity_score=regime_data.mean_reversion_score * confidence,
    ...
    def _generate_analysis_summary(self):
        ...
        summary["opportunity_summary"] = {
            ...
            "average_opportunity_score": total_opportunity_score / len(self.strategy_opportunities) if self.strategy_opportunities else 0
        }
        
        # Top opportunities (sorted by score)
        top_opps = sorted(self.strategy_opportunities, 
                         key=lambda x: x.opportunity_score, reverse=True)[:10]
        ...
        if self.strategy_opportunities:
            ...
            avg_return = np.mean([opp.expected_return for opp in self.strategy_opportunities])
            
            summary["risk_assessment"] = {
                "average_risk": avg_risk,
                "average_expected_return": avg_return,
                "risk_return_ratio": avg_return / avg_risk if avg_risk > 0 else 0,
                "high_risk_opportunities": len([opp for opp in self.strategy_opportunities if opp.risk_level > 0.25])
            }
    ...
    if result["status"] == "success":
        ...
        if "top_opportunities" in summary and summary["top_opportunities"]:
            ...
            for i, opp in enumerate(summary["top_opportunities"][:5]):
                print(f"  {i+1}. {opp['symbol']} ({opp['timeframe']}) - {opp['strategy']}")
                print(f"     Score: {opp['score']:.3f}, Expected Return: {opp['expected_return']:.1%}")
                print()
