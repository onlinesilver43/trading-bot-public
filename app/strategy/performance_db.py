    ...
    def record_trade(self, trade: TradeRecord) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                logger.info(
                    f"Recorded trade: {trade.strategy_name} {trade.signal} {trade.symbol}"
                )
                return True
    ...
    def calculate_performance(
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # buy_trades = [t for t in trades if t[0] == "buy"]  # Not currently used
                # sell_trades = [t for t in trades if t[0] == "sell"]  # Not currently used
                ...
                if len(wins) + len(losses) > 0:
                    returns = wins + [-loss for loss in losses]
    ...
    def get_performance_summary(
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if strategy_name:
                    cursor.execute(
                        "SELECT * FROM performance WHERE strategy_name = ?",
                        (strategy_name,),
                    )
                else:
                    cursor.execute(
                        "SELECT * FROM performance ORDER BY strategy_name, symbol, timeframe"
                    )
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                return {
                    ...
                    "regime_performance": [
                        {"regime": row[0], "trades": row[1], "avg_price": row[2]}
...
if __name__ == "__main__":
    ...
    metrics = db.calculate_performance(
        "SMA_Crossover", "BTC/USDT", "1h", "2024-01-01", "2024-12-31"
    )
