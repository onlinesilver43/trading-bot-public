...
class ProductionDataConnector:
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"❌ Production server connection failed: {response.status_code}"
                )
                return False
    ...
    async def get_available_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            manifest_response = await asyncio.get_event_loop().run_in_executor(
                None,
                self.session.get,
                f"{self.production_url}{self.endpoints['history_manifest']}",
            )

            if manifest_response.status_code != 200:
                logger.error(
                    f"Failed to get history manifest: {manifest_response.status_code}"
                )
                return {}
            ...
            status_response = await asyncio.get_event_loop().run_in_executor(
                None,
                self.session.get,
                f"{self.production_url}{self.endpoints['history_status']}",
            )

            if status_response.status_code == 200:
                status = status_response.json()
                logger.info(
                    f"Data directory: {status.get('data_directory', 'Unknown')}"
                )
        ...
        try:
            ...
            if data_response.status_code == 200:
                ...
                if all(col in data.columns for col in required_cols):
                    ...
                    data = (
                        data.sort_values("timestamp").tail(limit).reset_index(drop=True)
                    )
    ...
    async def download_data_sample(
        self, symbol: str, timeframe: str, output_dir: str = "data_samples"
    ) -> str:
    ...
    if await connector.test_connection():
        ...
        if "error" not in summary:
            ...
            if summary["available_symbols"]:
                ...
                timeframe = (
                    summary["available_timeframes"][0]
                    if summary["available_timeframes"]
                    else "1h"
                )
