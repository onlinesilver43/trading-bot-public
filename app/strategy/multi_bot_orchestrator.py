...
class MultiBotOrchestrator:
    ...
    def __init__(
        ...
        self.strategy_switching_threshold = (
            0.15  # 15% performance difference triggers switch
        )
    ...
    async def assign_strategies_to_bots(
        ...
        for allocation in sorted_allocations:
            ...
            if best_bot:
                ...
                logger.info(
                    "Assigned strategy %s to bot %s with %.2f capital",
                    strategy_name,
                    best_bot,
                    capital_amount,
                )
    ...
    def _find_best_bot_for_strategy(
        self, strategy_name: str, capital_amount: float
    ) -> Optional[str]:
    ...
    def _calculate_bot_strategy_compatibility(
        self, bot_config: BotConfig, strategy_name: str, capital_amount: float
    ) -> float:
        ...
        position_capacity = 1.0 - (
            self.bot_states[bot_config.bot_id].active_positions
        ...
        try:
            ...
            coordination_record = {
                ...
                "active_bots": len(
                    [
                        r
                        for r in coordination_results.values()
            ...
            logger.info(
                "Bot coordination completed: %d/%d bots active",
                coordination_record["active_bots"],
                coordination_record["total_bots"],
            )
    ...
    async def _coordinate_bot(self, bot_id: str, config: BotConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            if execution_result.get("status") == "success":
                ...
                current_state.error_message = execution_result.get(
                    "error", "Unknown error"
                )
    ...
    async def _should_switch_strategy(
        self, bot_id: str, current_assignment: StrategyAssignment
    ) -> bool:
        ...
        try:
            ...
            for alt_strategy in alternative_strategies:
                alt_performance = await self.performance_db.get_strategy_metrics(
                    alt_strategy
                )
                ...
                if alt_performance:
                    ...
                    if performance_diff > self.strategy_switching_threshold:
                        logger.info(
                            "Strategy switch recommended: %s -> %s (diff: %.2f%%)",
                            current_assignment.strategy_name,
                            alt_strategy,
                            performance_diff * 100,
                        )
                        return True
    ...
    async def _switch_bot_strategy(
        self, bot_id: str, current_assignment: StrategyAssignment
    ):
        ...
        try:
            ...
            for alt_strategy in alternative_strategies:
                performance = await self.performance_db.get_strategy_metrics(
                    alt_strategy
                )
                if performance:
    ...
    async def _execute_bot_strategy(
        self, bot_id: str, assignment: StrategyAssignment
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if strategy_name == "sma_crossover":
                execution_result["actions"] = [
                    "Analyzing SMA signals",
                    "Checking entry conditions",
                ]
            elif strategy_name == "mean_reversion":
                execution_result["actions"] = [
                    "Calculating mean reversion levels",
    ...
    async def run_orchestration_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            strategy_assignments = await self.assign_strategies_to_bots(
                portfolio_allocations
            )
    ...
    def get_orchestrator_status(self) -> Dict[str, Any]:
        ...
        active_bots = len(
            [s for s in self.bot_states.values() if s.status == BotStatus.ACTIVE]
        )
        ...
        return {
            ...
            "last_coordination": (
                self.coordination_history[-1] if self.coordination_history else None
            ),
        }
    ...
    def save_orchestrator_state(self, filename: str = None) -> str:
        ...
        state_data = {
            ...
            "bots": {
                ...
                for bot_id, (config, state) in zip(
                    self.bots.items(), self.bot_states.values()
                )
            },
            "strategy_assignments": {
                bot_id: {
...
async def main():
    ...
    for bot_id, assignment in result["strategy_assignments"].items():
        print(
            f"  {bot_id:<20} {assignment['strategy']:<15} {assignment['allocation']:<8} ${assignment['capital']:>8,.2f}"
        )

    print("\n📈 BOT STATES:")
    for bot_id, state in result["bot_states"].items():
        print(
            f"  {bot_id:<20} {state['status']:<12} {state['current_strategy']:<15} ${state['allocated_capital']:>8,.2f}"
        )
