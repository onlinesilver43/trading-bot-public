...
class MasterAgent:
    ...
    async def analyze_market_regime(self) -> MarketRegime:
        ...
        try:
            ...
            logger.info(
                "Market regime detected: %s (confidence: %.2f)",
    ...
    def calculate_strategy_scores(
        ...
        for strategy_name, perf in performance_data.items():
            ...
            regime_multiplier = self._get_regime_multiplier(
                config.strategy_type, market_regime
            )
    ...
    def _get_regime_multiplier(
        self, strategy_type: StrategyType, regime: MarketRegime
    ) -> float:
    ...
    def calculate_portfolio_allocation(
        self, strategy_scores: Dict[str, float], available_capital: float
    ...
    def _calculate_confidence(
        self, score: float, config: StrategyConfig, regime: MarketRegime
    ) -> float:
    ...
    def _generate_allocation_reasoning(
        ...
        if allocation_pct == 0:
            return (
                f"{strategy_name}: Insufficient capital or performance for allocation"
            )
        ...
        if confidence > 0.8:
            reasoning += (
                " - High confidence based on strong performance and market alignment"
            )
        elif confidence > 0.6:
    ...
    async def execute_portfolio_rebalancing(
        ...
        try:
            ...
            if total_allocated > self.total_capital:
                logger.warning(
                    "Total allocation exceeds available capital: %.2f > %.2f",
                    total_allocated,
                    self.total_capital,
                )
                return False
            ...
            logger.info(
                "Portfolio rebalancing completed: %.2f allocated across %d strategies",
                total_allocated,
                len(allocations),
            )
    ...
    async def run_optimization_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            strategy_scores = self.calculate_strategy_scores(
                performance_data, market_regime
            )
            ...
            allocations = self.calculate_portfolio_allocation(
                strategy_scores, available_capital
            )
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_allocations": (
                self.allocation_history[-5:] if self.allocation_history else []
            ),
            "performance_history": (
...
async def main():
    ...
    for alloc in result["allocations"]:
        print(
            f"  {alloc['strategy']:<15} {alloc['allocation']:<8} ${alloc['capital']:>8,.2f}"
        )
    ...
    print(
        f"🎯 System Status: {status['active_strategies']}/{status['total_strategies']} strategies active"
    )
