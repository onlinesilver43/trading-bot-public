...
class BacktestingEngine:
    ...
    def run_backtest(self, data: List[OHLCVData]) -> BacktestResult:
        ...
        logger.info(f"Backtest completed: {result.total_trades} trades, ${result.total_pnl:,.2f} PnL")
        return result
    
    def _get_market_regime(self, data: List[OHLCVData], current_index: int) -> Dict[str, Any]:
    ...
    def _generate_signal(self, data: List[OHLCVData], current_index: int, 
                        regime_info: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            signal, reason, cooldown_ok, sep = decide(
                fast_sma, slow_sma, closes[-1], config, 
                self.entry_time, data[current_index].timestamp, 86400000  # Daily timeframe
            )
            ...
            if regime_info['regime'] == 'sideways' and abs(regime_info['trend']) < 0.005:
    ...
    def _execute_trade(self, signal: str, candle: OHLCVData, regime_info: Dict[str, Any]):
    ...
    def _close_position(self, candle: OHLCVData, reason: str):
        ...
        logger.info(f"Closed {self.position} position: ${self.entry_price:.2f} -> ${exit_price:.2f}, PnL: {pnl:.2%}")
    ...
    def _update_equity_curve(self, current_price: float):
        ...
        if self.position == "long":
            position_value = self.current_capital * self.config.position_size
            current_value = position_value * (current_price / self.entry_price) if self.entry_price > 0 else position_value
...
def run_strategy_backtest(strategy_name: str, symbol: str, timeframe: str, 
                         days: int = 365, initial_capital: float = 10000.0) -> BacktestResult:
