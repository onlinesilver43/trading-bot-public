...
class CollectedDataConnector:
    ...
    async def get_available_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stdout, stderr, code = self.run_ssh_command(
                "cat /srv/trading-bots/history/manifest.json | jq '.'"
            )
            if code != 0:
            ...
            try:
                ...
                logger.info(
                    f"Found {manifest.get('statistics', {}).get('total_files', 0)} data files"
                )

                return {"manifest": manifest, "timestamp": datetime.now().isoformat()}
    ...
    async def get_symbol_data(
        ...
        try:
            ...
            command = (
                f"ls {self.base_path}/{symbol}/{timeframe}/{year_month}/ | head -1"
            )
            stdout, stderr, code = self.run_ssh_command(command)
            ...
            try:
                ...
                if all(col in df.columns for col in required_cols):
                    ...
                    logger.info(
                        f"✅ Processed {len(df)} data points for {symbol} {timeframe}"
                    )
                    return df
                else:
    ...
    async def get_market_data_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_size_mb": round(
                    statistics.get("total_size_bytes", 0) / (1024 * 1024), 2
                ),
...
async def main():
    ...
    if btc_data is not None:
        print("✅ BTCUSDT 1h data loaded successfully!")
    ...
    if eth_data is not None:
        print("✅ ETHUSDT 1h data loaded successfully!")
